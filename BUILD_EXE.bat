...
if not exist "%ROOT%\tools\icon.ico" echo [INFO] Pas d icone (tools\icon.ico) - icone par defaut.
...
echo [2/2] Succes !
echo.
echo Fichiers generes dans : %ROOT%\dist_electron\
echo.
for %%F in ("%ROOT%\dist_electron\*.exe") do echo   %%~nxF
echo.
pause
